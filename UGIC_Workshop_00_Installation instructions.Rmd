---
title: "UGIC R Workshop  00 Installing R and RStudio"
knit: (function(input_file, encoding) {
  out_dir <- 'docs';
  rmarkdown::render(input_file,
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'UGIC_Workshop_00_Installation.html'))})
author: | 
  | Simon Brewer
...
NOTE: These labs were developed and tested using R 4.3.2. The current version of R as of today is 4.4.0
...
You can download RStudio for free from the [RStudio website][rstudioid] (now called `posit.co`). Click on the 'Download RStudio' button, then find the download button under RStudio Desktop. Click this, and you should see a new page, with a download button for your operating system. Once this has downloaded, run the installer, and accept any defaults. Once you've installed RStudio, you can open it like any other program on your computer — usually by clicking an icon on your desktop/start menu/applications folder.

### The RStudio GUI

When you open RStudio, a window appears with three panels in it, as in the figure below. The largest pane is a console window. This is where you’ll run your R code and see results. The console window is exactly what you’d see if you ran R from a UNIX console or the Windows or Mac GUIs. Everything else you see is unique to RStudio. Hidden in the other panes are a text editor, a graphics window, a debugger, a file manager, and much more. In the first session we'll start to explore these a little more. 
