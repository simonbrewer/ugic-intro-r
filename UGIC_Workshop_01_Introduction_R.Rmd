...
knit: (function(input_file, encoding) {
 ...
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'UGIC_Workshop_01_Introduction_R.html'))})
author: | 
  | Simon Brewer
...
In this lab, we will introduce the basic functionality of R, together with some simple plotting functions. We will be using a dataset of morphological measurements of three species of penguin from the file *penguins.csv*. You will need to download this from the Google Drive [here](https://drive.google.com/drive/folders/1sE--X8YfUxq5gZPNQz4Zjk6uBeAcIBn3?usp=sharing).
...
If this all seems a little foreign to you, don't worry - there will be plenty of opportunities to practice this over the day, but ask if you would like to see this demonstrated. Understanding the directory structure is very important in being able to manage your files both for this training session and any analysis you will do later. 
...
And press 'Enter', and R will tell you, not too surprisingly, that 2+2=4. The spacing is not important: you could equally enter `2 + 2` or `2+ 2` and get the same result. The `[1]` before the output is a vector index. It refers to the first value in the *output* vector (here a vector of length 1). We'll look at indexes in more detail later.
...
Or to get the square of a number:
...
So far, these commands have runs some calculations and displayed the results in the console. If you need to store any R output for further use, you will need to assign the output to to a variable. There are two assignment operators in R `<-` and `=`. These are interchangeable, and you will see both used in R examples. To store the results of the previous commands:
...
Let's read in the data from the Penguin file (*penguins.csv*). Make sure you've downloaded it from the Google drive and copied it to the correct folder - and let me know if you can't load it. CSV files can be read in using the `read.csv()` function:
...
The first part of this code (`penguin <-`) tells R to store the data read in from the file in a *data frame* called `penguin` (you can call this anything, but generally it's advisable to use a name that will help remind you what the data is). To print out the contents of any object in R, simply type the name of that object at the command prompt.
...
You can write out dataframes to csv files with `write.csv()`. As an example, we'll write the penguin data back out to a new file (normally, we'd only do this after transforming or working with the data). Once you've run this, check the `data` folder to make sure the new file has been created.
...
If you only provide one index, then it will extract all values for that row/column:
...
Where dataframes differ is in the use of the `$` notation. This allows you to access individual columns or variables. For example, to extract the bill length variable:
...
The advantage of this is that you can access a column, even if you don't know the column number. You can also combine this with the vector indexing (see above) to access specific values within a vector:
...
penguin$bill_length_mm[(penguin$species == 'Adelie')] 	# Bill lengths for "Adelie" penguins
...
You can add multiple conditions in the filter function. For example, to filter for male penguins with a body mass of over 4000 g:
...
To filter for male penguins with a body mass of over 4000 g, and are from Biscoe island: 
...
Note that there is also a `slice()` function, which simply extracts rows according to their position - this is the equivalent to the row indexing we looked at before:
...
We'll look at a couple of other **dplyr** functions shortly, but first, we'll explore what an R function is.
...
R has a large number of in-built functions. This section is designed to simply introduce you to the some basic functions for estimating simple univariate statistics We'll start by simply calculating the mean of the bill length values

```{r}
mean(penguin$bill_length_mm)
```

This returns the value `NA`, rather than a mean length. So what went wrong? In the original set of data, there are some missing values, also denoted by `NA`. For example, here are the first 10 values: 

```{r}
penguin$bill_length_mm[1:15]
```

R's default for most functions is to not calculate values when there are missing observations. This is really to alert you to the fact that the data are incomplete, and the value you would obtain might be biased. You can overrule this by adding the argument `na.rm=TRUE` to most functions. This removes `NA`s and calculates the value with whatever is leftover.

Functions to describe the central tendency:

```{r}
mean(penguin$bill_length_mm, na.rm = TRUE)
median(penguin$bill_length_mm, na.rm = TRUE)
```
...
var(penguin$bill_length_mm, na.rm = TRUE) ## Variance
min(penguin$bill_length_mm, na.rm = TRUE)
...
```{r}
penguin %>% 
    summarize(avg_body_mass = mean(body_mass_kg, na.rm = TRUE))
```
...
```{r}
penguin %>% 
    summarize(count_islands = n_distinct(island),
...
Let's clean up this plot a little by specifying the axis labels and a title (`main`):
...
In order to understand how `ggplot` makes a figure, we need to establish what the fundamental parts of data graphics are. These can include:
...
When making a `ggplot` figure, we generally start by creating the base figure. To do this we need to tell the function where the data is coming from, and the base aesthetic (i.e. which variable is `x`, which is `y`, are we using a variable to define colors?). To remake the first scatter plot with `ggplot()`, we start by doing the following:
...
Note that, in addition to the points, **ggplot2** has several other geometries for lines, barplots, histograms, polygons, and even maps (we'll look at this later). Here's just a few examples:
