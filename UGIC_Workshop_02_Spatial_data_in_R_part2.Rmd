...
In the previous lab, we looked at making maps in base R (with the `plot()` function and an `sf` object) as well as using **ggplot2**. More recently, there has been something of an explosion in mapping packages, including interactive maps. We'll look here at three of these: **tmap**, **mapview** and **leaflet**. 
```{r}
