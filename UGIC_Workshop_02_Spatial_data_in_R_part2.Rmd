...
If the CRS is not set, you can set it using EPSG code. For example, the following code would set the CRS to WGS84 (**don't run this as the CRS is already defined**)
...
As the Landsat data is in a UTM projection (10N), we'll need to reproject the CA place data to match:
...
random_pnts <- st_sample(bethel, size = 50)
